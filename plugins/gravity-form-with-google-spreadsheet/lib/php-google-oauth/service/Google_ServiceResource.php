 ...
 *
 */
class GFGS_Google_ServiceResource {
  ...
  /** @var GFGS_Google_Service $service */
  private $service;
   ...
   * @param $name
   * @param $arguments
   * @return GFGS_Google_HttpRequest|array
   * @throws GFGS_Google_Exceptions
   */
  public function __call($name, $arguments) {
    if (! isset($this->methods[$name])) {
      throw new GFGS_Google_Exceptions("Unknown function: {$this->serviceName}->{$this->resourceName}->{$name}()");
    }
    ...
    foreach ($parameters as $key => $val) {
      if ($key != 'postBody' && ! isset($method['parameters'][$key])) {
        throw new GFGS_Google_Exceptions("($name) unknown parameter: '$key'");
      }
    }
    if (isset($method['parameters'])) {
      foreach ($method['parameters'] as $paramName => $paramSpec) {
        if (isset($paramSpec['required']) && $paramSpec['required'] && ! isset($parameters[$paramName])) {
          throw new GFGS_Google_Exceptions("($name) missing required param: '$paramName'");
        }
    ...
    if (isset($method['mediaUpload'])) {
      $media = GFGS_Google_MediaFileUpload::process($postBody, $parameters);
      if ($media) {
    ...
    $url = GFGS_Google_REST::createRequestUri($servicePath, $method['path'], $parameters);
    $httpRequest = new GFGS_Google_HttpRequest($url, $method['httpMethod'], null, $postBody);
    if ($postBody) {
      ...
      if (isset($contentType) && $contentType) {
        ...
        $contentTypeHeader['content-length'] = GFGS_Google_Utils::getStrLen($postBody);
      }
      $httpRequest->setRequestHeaders($contentTypeHeader);
    }

    $httpRequest = GFGSC_Google_Client_Connector::$auth->sign($httpRequest);
    if (GFGSC_Google_Client_Connector::$useBatch) {
      return $httpRequest;
    }
    ...
    return GFGS_Google_REST::execute($httpRequest);
  }
