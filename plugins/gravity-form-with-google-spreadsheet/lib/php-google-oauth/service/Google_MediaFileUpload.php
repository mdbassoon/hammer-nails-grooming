...
class GFGS_Google_MediaFileUpload {
  ...
  public static function getUploadType($meta, &$payload, &$params) {
    if (isset($params['mediaUpload'])
        && get_class($params['mediaUpload']['value']) == 'GFGS_Google_MediaFileUpload') {
  ...
  public function nextChunk(GFGS_Google_HttpRequest $req) {
    ...
    $httpRequest = new GFGS_Google_HttpRequest($this->resumeUri, 'PUT', $headers, $data);
    $response = GFGSC_Google_Client_Connector::$io->authenticatedRequest($httpRequest);
    ...
    if (308 == $code) {
      ...
      return GFGS_Google_REST::decodeHttpResponse($response);
    }
  }

  private function getResumeUri(GFGS_Google_HttpRequest $httpRequest) {
    $result = null;
    $body = $httpRequest->getPostBody();
    if ($body) {
      $httpRequest->setRequestHeaders(array(
        'content-type' => 'application/json; charset=UTF-8',
        'content-length' => GFGS_Google_Utils::getStrLen($body),
        'x-upload-content-type' => $this->mimeType,
        'expect' => '',
      ));
    }

    $response = GFGSC_Google_Client_Connector::$io->makeRequest($httpRequest);
    $location = $response->getResponseHeader('location');
    $code = $response->getResponseHttpCode();
    if (200 == $code && true == $location) {
      return $location;
    }
    throw new GFGS_Google_Exceptions("Failed to start the resumable upload");
  }
}