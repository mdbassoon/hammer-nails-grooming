...
class GFGS_Google_REST {
   ...
   *
   * @param GFGS_Google_HttpRequest $req
   * @return array decoded result
   * @throws GFGS_Google_ServiceException on server side error (ie: not authenticated,
   *  invalid or malformed post body, invalid url)
   */
  static public function execute(GFGS_Google_HttpRequest $req) {
    $httpRequest = GFGSC_Google_Client_Connector::$io->makeRequest($req);
   ...
   * @static
   * @throws GFGS_Google_ServiceException
   * @param GFGS_Google_HttpRequest $response The http response to be decoded.
   * @return mixed|null
   */
  public static function decodeHttpResponse($response) {
    ...
    if ($code != '200' && $code != '201' && $code != '204') {
      ...
      throw new GFGS_Google_ServiceException($err, $code, null, $decoded['error']['errors']);
    }
    ...
    if ($code != '204') {
      ...
      if ($decoded === null || $decoded === "") {
        throw new GFGS_Google_ServiceException("Invalid json in service response: $body");
      }
    }
    return $decoded;
  }
  ...
  static function createRequestUri($servicePath, $restPath, $params) {
    ...
    if (count($uriTemplateVars)) {
      $uriTemplateParser = new GFGS_URI_Template_Parser($requestUrl);
