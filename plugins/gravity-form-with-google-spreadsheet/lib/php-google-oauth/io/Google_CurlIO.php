...
class GFGS_Google_CurlIO implements GFGS_Google_IO {
   ...
   * @return GFGS_Google_HttpRequest The resulting HTTP response including the
   * responseHttpCode, responseHeaders and responseBody.
   */
  public function authenticatedRequest(GFGS_Google_HttpRequest $request) {
    $request = GFGSC_Google_Client_Connector::$auth->sign($request);
   ...
   *
   * @param GFGS_Google_HttpRequest $request the http request to be executed
   * @return GFGS_Google_HttpRequest http request with the response http code, response
   * headers and response body filled in
   * @throws GFGS_Google_IOException on curl or IO error
   */
  public function makeRequest(GFGS_Google_HttpRequest $request) {
    // First, check to see if we have a valid cached version.
    $cached = $this->getCachedRequest($request);
    if ($cached !== false) {
      if (GFGS_Google_CacheParser::mustRevalidate($cached)) {
        $addHeaders = array();
    ...
    if ($curlErrorNum != CURLE_OK) {
      throw new GFGS_Google_IOException("HTTP Error: ($respHttpCode) $curlError");
    }
   ...
   */
  public function setCachedRequest(GFGS_Google_HttpRequest $request) {
    // Determine if the request is cacheable.
    if (GFGS_Google_CacheParser::isResponseCacheable($request)) {
      GFGSC_Google_Client_Connector::$cache->set($request->getCacheKey(), $request);
      return true;
    }
   ...
   * @visible for testing.
   * @param GFGS_Google_HttpRequest $request
   * @return GFGS_Google_HttpRequest|bool Returns the cached object or
   * false if the operation was unsuccessful.
   */
  public function getCachedRequest(GFGS_Google_HttpRequest $request) {
    if (false == GFGS_Google_CacheParser::isRequestCacheable($request)) {
      false;
    }

    return GFGSC_Google_Client_Connector::$cache->get($request->getCacheKey());
  }
   ...
   * @param GFGS_Google_HttpRequest $request
   * @return GFGS_Google_HttpRequest Processed request with the enclosed entity.
   */
  public function processEntityRequest(GFGS_Google_HttpRequest $request) {
